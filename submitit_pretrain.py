...
def parse_args():
    ...
    parser.add_argument(
        "--timeout", default=4320, type=int, help="Duration of the job"
    )
    parser.add_argument(
        "--job_dir",
        default="",
...
def get_shared_folder() -> Path:
    user = os.getenv("USER")
    if Path("/experiments/checkpoints/").is_dir():
        p = Path(f"/experiments/checkpoints/{user}/experiments")
...
if __name__ == "__main__":
    setproctitle.setproctitle(os.getenv("PROC_TITLE", "run_mae"))
    main()
